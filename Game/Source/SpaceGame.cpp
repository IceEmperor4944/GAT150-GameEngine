...
void SpaceGame::Update(float dt) {
    auto start = Factory::Instance().Create<Actor>("start");
    auto player = Factory::Instance().Create<Actor>("player");
    auto room = Factory::Instance().Create<Actor>("room1");
    ...
    case State::Title:
        ...
        start->transform.position = Vector2{ 480, 480 };
        m_scene->AddActor(std::move(start), true);
    ...
    case State::StartGame:
        m_engine->GetAudio().PlaySound("Sounds/song.wav");
    ...
    case State::StartLevel:
        ...
        room->transform.position = Vector2{ 32, 32 };
        m_scene->AddActor(std::move(room), true);
    ...
    case State::Game:
        ...
        if (spawnTimer <= 0) {
            ...
            bat->transform.position = Vector2{ 700, 60 };
        ...
        if (m_scene->GetActor<Actor>()->destroyed) {
